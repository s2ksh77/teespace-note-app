...
var ChapterStore = mobx.observable((_observable$2 = {
  ...
  var items = JSON.parse(localStorage.getItem('NoteSortData_' + NoteStore.getChannelId()));
  var chapter = items.find(function (chapter) {
    return chapter.id === chapterId;
  });
    ...
    switch (type) {
      ...
      case 'failUploadByFileNameLen':
        eventList.push(function (e) {
          e.stopPropagation();
          NoteStore.setModalInfo(null);
          EditorStore.setIsFileFilteredByNameLen(false); // if (EditorStore.uploadDTO.length === EditorStore.uploadLength) handleUpload();
        });
        break;
    }
