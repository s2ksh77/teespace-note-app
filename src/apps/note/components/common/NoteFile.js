...
export const exportPageAsTxt = async (noteId) => {
    ...
    // PageStore.exportPageTitle = dto.note_title
    let returnData = `<span style="font-size:24px;">${i18n.t('NOTE_EXPORT_TITLE')} : ${dto.note_title}</span><br />${dto.note_content}`;
...
export const exportChapterAsTxt = async (chapterTitle, chapterId) => {
    ...
    if (noteList.length > 0) {
        noteList.forEach((page, idx) => {
            returnData += `<span style="font-size:24px;">${i18n.t('NOTE_EXPORT_TITLE')} : ${page.note_title}</span>
      <br />
      ${page.note_content}
