...
class NoteRepository {
  async getChapterList(chId: string): Promise<any> {
    console.log(chId);
    try {
  ...
  // 차이가 뭘까?
  // { chId, noteId, onClick, pageInfo .....}:{ chId:string, noteId:string , onClick: Func , pageInfo : PageModel ...}
  // { chId: string, noteId: string, onClick: Func, pageInfo : PageModel }
  async getNoteInfoList(chId: string, noteId: string): Promise<any> {
    try {
      return await API.Get(
        `note-api/noteinfo?action=List&note_id=${noteId}&note_channel_id=${chId}`,
      );
    } catch (e) {
      throw Error(JSON.stringify(e));
    }
  ...
  getNoteTagList({ noteId }: { noteId: string }): Promise<any> {
    return API.Get(
